class Player {
    ...
    move(y_target, x_target) {
        ...
        const direction_y = Math.sign(y_target - current_y)
        const direction_x = Math.sign(x_target - current_x)
    ...
    __move(y_target, x_target, marker, speed, y_direction, x_direction) {
        ...
        console.log(y_target, x_target)
        console.log(next_y, next_x)
        console.log(y_direction, x_direction)
        if (y_direction > 0) {
            if (next_y > y_target) {
                next_y = y_target
                reached_y = true
